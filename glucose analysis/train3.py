...
def main(
    ...
    min_glucose['y'] = min_glucose['Glucose']
    ...
    optimizer = torch.optim.Adam(mv_net.parameters(), lr=learning_rate)
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'graphs/glucose_pred3_{train_episodes}e.png')

    classified = 0
    correct_low = 0
    low = 0
    for i in range(len(y_test_pred)):
        if (y_test_pred[i] < low_threshold and y_test[i] < low_threshold) or (y_test_pred[i] >= low_threshold and y_test[i] >= low_threshold):
            classified += 1
        if (y_test[i] < low_threshold):
            low += 1
            if y_test_pred[i] < low_threshold:
                correct_low += 1
    print(f'{classified/len(y_test_pred)*100}% Correct classification')
    print(f'{correct_low/low*100}% Correct Low event prediction out of {low} low events.')
